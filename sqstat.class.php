...
class squidstat{
	...
	*/
	...
	function makeHtmlReport($data,$resolveip=false,$hosts_array=array(),$use_js=true){
		...
		'Статистика в реальном времени для'.$servers.' прокси сервера ('.$data["server_version"].').<br/>'.
		'Автоматическое обновление: <input name="refresh" type="text" size="4" value="'.$refresh.'"/> сек. <input type="submit" value="Обновить"/> <input name="stop" type="submit" value="Стоп"/> Создано: <tt>'.date("h:i:s d/m/Y").'</tt><br/>'.
		'</div>'.
		'<table class="result" align="center" width="100%" border="0">'.
		'<tr>'.
		'<th>'.$group_by_name.'</th><th>Адрес</th>'.
		($this->use_sessions?'<th>Тек. Скорость</th><th>Средн. Скорость</th>':'').
		'<th>Размер</th><th>Время</th>'.
		'</tr>';
		$ausers=$acon=0;
		unset($session_data);
		...
		$stat_row='';
		if($this->use_sessions){
			$stat_row.=sprintf("<tr class=\"total\"><td><b>Всего:</b></td><td align=\"right\" colspan=\"5\"><b>%d</b> пользователей и <b>%d</b> соединений @ <b>%01.2f/%01.2f</b> KB/s (CURR/AVG)</td></tr>",
			$ausers, $acon, $total_curr, $total_avg);
		}
		else {
			$stat_row.=sprintf("<tr class=\"total\"><td><b>Всего:</b></td><td align=\"right\" colspan=\"5\"><b>%d</b> пользователей и <b>%d</b> соединений</td></tr>",
			$ausers, $acon);
		}
		if($ausers==0){
